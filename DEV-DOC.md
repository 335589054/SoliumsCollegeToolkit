...
- [x] 「快捷入口」小组件的内容与首页快捷入口共用 `quick_tools` / `links.isHomeCard` 配置
...
| `quick_tools` | String | 快捷入口工具 key JSON（同时控制首页与桌面「快捷入口」小组件） |
...
> v1.0.1 起底部菜单与二级页面切换统一使用水平滑动动画。

> 首页「快捷入口」是**工具箱内工具 + 便捷链接**的聚集地，与「便捷链接收藏」统一管理，避免重复维护。
...
```
首页
├─ 首页快捷入口（工具入口 + 链接入口统一管理，首页可点击跳转）
├─ 桌面快捷入口小组件（勾选内容同步显示在桌面小组件）
...
内置 **12 套**预设主题，均采用扁平化设计，按**色调排序**，同时定义浅色 + 深色两套色值；v1.0.1 起支持把主题颜色按 UI 角色（主色、页面背景、卡片、文字、边框等）字段化自定义，并支持 .sthm 主题包导入 / 导出；v1.0.3 起颜色选择器使用色调 / 饱和度 / 亮度滑杆。
