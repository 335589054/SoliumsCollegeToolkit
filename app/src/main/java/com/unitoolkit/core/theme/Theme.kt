package com.unitoolkit.core.theme
...
private fun parseColorOr(hex: String?, fallback: Color): Color =
    hex?.takeIf { it.isNotBlank() }?.let { runCatching { Color(android.graphics.Color.parseColor(it)) }.getOrNull() } ?: fallback
...
private fun rolePalette(
    ...
    fun c(role: String): Color {
        ...
        return parseColorOr(hex, defaults[role] ?: Color.Black)
    }
    return AppPalette(
...
fun AppTheme.toCustomTheme(id: String, name: String): CustomTheme {
    ...
    return CustomTheme(
        ...
        colors = ThemeRoleDefs.associate { role ->
            role.key to com.unitoolkit.core.model.ThemeRoleColors(
