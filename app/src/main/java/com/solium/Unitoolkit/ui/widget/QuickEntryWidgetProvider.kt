...
class QuickEntryWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val enabledTools = app.settings.widgetQuickTools.first()
    ...
    private fun buildEntries(enabledTools: List<String>, homeLinks: List<com.solium.Unitoolkit.core.database.LinkEntity>): List<QuickEntryView> {
        // 按设置中保存的有序展示顺序生成工具入口；链接追加在工具之后。
        val tools = enabledTools.mapNotNull { key -> quickTools.firstOrNull { it.key == key } }
            .map { QuickEntryView(it.icon, it.route) }
...
internal fun openRoutePendingIntent(context: Context, route: String, requestCode: Int): PendingIntent {
    val intent = Intent(context, MainActivity::class.java)
        .setAction("com.solium.Unitoolkit.action.OPEN_QUICK_ENTRY")
